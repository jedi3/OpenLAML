...
public class LogisticRegression extends Classifier
{
    ...
    public static void main(final String[] args) {
        ...
        final int regularizationType = 2;
        ...
        Time.tic();
        logReg.train();
        Printer.fprintf("Elapsed time: %.3f seconds.%n", Time.toc());
    ...
    public LogisticRegression() {
        this.regularizationType = 0;
        this.lambda = 0.0;
    }
    
    public LogisticRegression(final int regularizationType, final double lambda) {
        this.regularizationType = regularizationType;
        this.lambda = lambda;
    }
    
    public LogisticRegression(final double epsilon) {
    ...
    public void train() {
        ...
        switch (this.regularizationType) {
            case 1: {
                AcceleratedProximalGradient.prox = new ProxL1(this.lambda);
                break;
            }
            case 2: {
                AcceleratedProximalGradient.prox = new ProxL2Square(this.lambda);
                break;
            }
            case 3: {
                AcceleratedProximalGradient.prox = new ProxL2(this.lambda);
                break;
            }
            case 4: {
                AcceleratedProximalGradient.prox = new ProxLInfinity(this.lambda);
                break;
            }
        }
        while (true) {
            if (this.regularizationType == 0) {
                flags = LBFGS.run(G, fval, this.epsilon, W);
            }
            else {
                flags = AcceleratedProximalGradient.run(G, fval, hval, this.epsilon, W);
            }
            if (flags[0]) {
                break;
            }
            ...
            switch (this.regularizationType) {
                case 1: {
                    hval = this.lambda * Matlab.sumAll(Matlab.abs(W));
                    break;
                }
                case 2: {
                    final double norm = Matlab.norm(W, "fro");
                    hval = this.lambda * norm * norm;
                    break;
                }
                case 3: {
                    hval = this.lambda * Matlab.norm(W, "fro");
                    break;
                }
                case 4: {
                    hval = this.lambda * Matlab.max(Matlab.max(Matlab.abs(W))[0])[0];
                    break;
                }
            }
            if (!flags[1]) {
