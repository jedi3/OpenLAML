...
public class KernelByColumns
{
    ...
    public static Matrix calcKernel(final String kernelType, final double kernelParam, final Matrix X1, final Matrix X2) {
        ...
        if (kernelType.equals("linear")) {
            ...
            final double[] AA = Matlab.sum(Matlab.times(X1, X1)).getPr();
            final double[] BB = Matlab.sum(Matlab.times(X2, X2)).getPr();
            final Matrix AB = X1.transpose().mtimes(X2);
            ...
            if (AB instanceof DenseMatrix) {
                ...
                for (int j = 0; j < N; ++j) {
                    for (int k = jc[j]; k < jc[j + 1]; ++k) {
                        ...
                        final int n2 = k;
                        array2[n2] /= Math.sqrt(AA[ir[k]] * BB[j]);
                    }
                }
            }
            K = AB;
            ...
            K = Matlab.pow(X1.transpose().mtimes(X2), kernelParam);
        }
        else if (kernelType.equals("rbf")) {
