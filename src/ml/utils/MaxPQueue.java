...
public class MaxPQueue<Key extends Comparable, K> implements Iterable<Key>
{
    private Comparable[] pq;
    private int N;
    private HashMap<Key, Integer> keyToIndexMap;
    
    public MaxPQueue(final int initCapacity) {
    ...
    public MaxPQueue(final Key[] keys) {
        this.keyToIndexMap = new HashMap<Key, Integer>();
        this.N = keys.length;
    ...
    public void update(final Key key, final K k) {
        ...
        if (index > 1 && this.less(index / 2, index)) {
            this.swim(index);
        }
        else {
            this.sink(index);
    ...
    private boolean isMaxHeap(final int k) {
        ...
        return (left > this.N || !this.less(k, left)) && (right > this.N || !this.less(k, right)) && (this.isMaxHeap(left) && this.isMaxHeap(right));
    }
    ...
    private class HeapIterator implements Iterator<Key>
    {
        ...
        public HeapIterator() {
            this.copy = new MaxPQueue<Key, K>(MaxPQueue.this.size());
            for (int i = 1; i <= MaxPQueue.this.N; ++i) {
                this.copy.insert((Key)MaxPQueue.this.pq[i]);
            }
        }
