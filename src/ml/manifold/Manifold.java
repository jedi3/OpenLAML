...
public class Manifold
{
    public static void main(final String[] args) {
        ...
        X = X.transpose();
        Printer.disp(X.getSubMatrix(0, 10, 0, 100));
        ...
        if (show) {
            Printer.disp(A.getSubMatrix(0, 4, 0, 4));
        }
        Time.tic();
        ...
        if (show) {
            Printer.disp(L.getSubMatrix(0, 4, 0, 4));
        }
        ...
        if (show) {
            Printer.display(LLR_text.getSubMatrix(0, 4, 0, 4));
        }
    }
    ...
    public static Matrix adjacencyDirected(final Matrix X, final String type, final double param, final String distFunc) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            final double[] IX = Matlab.sort(Z);
            if (type.equals("nn")) {
                for (int j = 0; j <= param; ++j) {
                    if (IX[j] != i) {
                        A.setEntry(i, (int)IX[j], Z.get(j) + Matlab.eps);
                    }
                }
            }
            else if (type.equals("epsballs") || type.equals("eps")) {
                for (int j = 0; Z.get(j) <= param; ++j) {
                    if (IX[j] != i) {
                        A.setEntry(i, (int)IX[j], Z.get(j) + Matlab.eps);
                    }
                }
    ...
    public static Matrix cosine(final Matrix A, final Matrix B) {
        final double[] AA = Matlab.sum(Matlab.times(A, A), 2).getPr();
        final double[] BB = Matlab.sum(Matlab.times(B, B), 2).getPr();
    ...
    public static Vector cosine(final Vector V, final Matrix B) {
        final double AA = Matlab.sum(Matlab.times(V, V));
        final double[] BB = Matlab.sum(Matlab.times(B, B), 2).getPr();
        final DenseVector AB = Matlab.full(B.operate(V));
        final int dim = AB.getDim();
        double v = 0.0;
        final double[] pr = AB.getPr();
        for (int j = 0; j < dim; ++j) {
            v = pr[j];
            pr[j] = 1.0 - v / Math.sqrt(AA * BB[j]);
        }
        return AB;
    }
    
    public static Matrix euclidean(final Matrix A, final Matrix B) {
        return Matlab.l2Distance(A, B);
    }
    
    public static Vector euclidean(final Vector V, final Matrix B) {
        return Matlab.l2Distance(V, B);
    }
    ...
    public static Matrix calcLLR(final Matrix X, final double NN, final String distFunc, final String kernelType, final double kernelParam, final double lambda) {
        ...
        final Matrix K = Kernel.calcKernel(kernelType, kernelParam, X);
        final int NSample = Matlab.size(X, 1);
        ...
        if (X instanceof DenseMatrix) {
            Xs = Matlab.denseMatrix2DenseRowVectors(X);
        }
        else if (X instanceof SparseMatrix) {
            Xs = Matlab.sparseMatrix2SparseRowVectors(X);
        }
        ...
        for (int i = 0; i < NSample; ++i) {
            ...
            x_i = Xs[i];
            k_i = Kernel.calcKernel(kernelType, kernelParam, new Vector[] { x_i }, neighborhood_X_i);
